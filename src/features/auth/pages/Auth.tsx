...
export default function Auth() {
  ...
  return (
    <main className="min-h-screen flex bg-background text-foreground relative overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-surface-sunken border-r border-border p-12 flex-col justify-between relative z-10">
        ...
        <Link to="/" className="flex items-center gap-2 group self-start">
          <div className="h-8 w-8 rounded-sm flex items-center justify-center overflow-hidden border border-border bg-card group-hover:border-primary/50 transition-colors">
        ...
        <div className="space-y-10 max-w-lg my-auto">
          <div className="space-y-4">
            <h2 className="text-4xl font-extrabold tracking-tight font-display text-foreground leading-tight">
          ...
          <div className="space-y-6">
            <div className="flex gap-4 items-start">
              <div className="h-8 w-8 rounded-sm bg-surface-raised border border-border flex items-center justify-center text-primary shrink-0"><Upload className="h-3.5 w-3.5" /></div>
              <div>
                <h4 className="text-sm font-semibold text-foreground font-display mb-1 flex items-center gap-1.5">Ingest Sources</h4>
                <p className="text-xs text-muted-foreground">Drag files or drop media links. Our parser indexes contents locally.</p>
              </div>
            </div>
            <div className="flex gap-4 items-start">
              <div className="h-8 w-8 rounded-sm bg-surface-raised border border-border flex items-center justify-center text-primary shrink-0"><Headphones className="h-3.5 w-3.5" /></div>
              <div>
                <h4 className="text-sm font-semibold text-foreground font-display mb-1 flex items-center gap-1.5">Synthesize Recap</h4>
                <p className="text-xs text-muted-foreground">Audio summaries compile automatically alongside notes & quiz modules.</p>
              </div>
            </div>
            <div className="flex gap-4 items-start">
              <div className="h-8 w-8 rounded-sm bg-surface-raised border border-border flex items-center justify-center text-primary shrink-0"><ListChecks className="h-3.5 w-3.5" /></div>
              <div>
                <h4 className="text-sm font-semibold text-foreground font-display mb-1 flex items-center gap-1.5">Grounded Dialogue</h4>
                <p className="text-xs text-muted-foreground">Chat with documents. Answers cite underlying source fragments directly.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center items-center px-6 sm:px-12 py-16 relative z-10">
        ...
        <div className="lg:hidden flex items-center gap-2 justify-center mb-10">
          ...
          <h1 className="text-xl font-semibold tracking-tight text-foreground font-display">Source.io</h1>
        </div>

        <div className="w-full max-w-sm">
          {/* Glass Form Card */}
          <div className="plate plate-registered p-8 rounded-sm relative">
            {signupSuccess ? (
              <div className="text-center py-4 space-y-6 animate-in fade-in zoom-in duration-300">
                <div className="h-16 w-16 mx-auto rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center text-primary">
              ...
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  {mode === "signup" && (
                    <div className="space-y-1.5">
                      <Label htmlFor="name" className="text-xs text-foreground/90">Display name</Label>
                      <Input 
                        id="name" 
                        value={displayName} 
                        onChange={(e) => setDisplayName(e.target.value)} 
                        placeholder="Your name" 
                        className="bg-surface-raised border-border/60 focus:border-primary/50 text-foreground placeholder:text-muted-foreground/70 rounded-sm text-sm"
                      />
                    </div>
                  )}
                  <div className="space-y-1.5">
                    <Label htmlFor="email" className="text-xs text-foreground/90">Email address</Label>
                    <Input 
                      id="email" 
                      type="email" 
                      value={email} 
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <Label htmlFor="password" className="text-xs text-foreground/90">Password</Label>
                    </div>
                    <Input 
                      id="password" 
                      type="password" 
                  ...
                  <Button 
                    type="submit" 
                    className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-semibold py-2.5 rounded-sm transition-colors mt-2" 
                    disabled={submitting}
                  >
                    {submitting ? (
                ...
                <div className="relative my-6">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-card px-2.5 text-xs text-muted-foreground font-mono">or continue with</span>
                  </div>
                </div>
                ...
                <Button
                  type="button"
                  variant="outline"
                  className="w-full bg-surface-raised border-border hover:bg-surface-elevated text-foreground font-medium py-2.5 rounded-sm flex items-center justify-center gap-2"
                  disabled={submitting}
                  onClick={async () => {
                    setSubmitting(true);
                    try {
